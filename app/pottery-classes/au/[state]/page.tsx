...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: `Find pottery and ceramics classes across ${stateLabel}, Australia. Browse studios by city offering wheel throwing, hand building, open studio memberships, and more.`,
    alternates: { canonical: `https://www.clayfinder.com/pottery-classes/au/${state}` },
  }
}
...
export default async function AuStateePage({ params }: Props) {
  const { state } = await params
    ...
    <main className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
        {cities.map(({ city, count }) => (
          <Link
            key={city}
            href={`/pottery-classes/au/${state}/${slugify(city)}`}
      ...
      <div className="border-t border-stone-200 pt-6 mt-10">
        <Link href="/pottery-classes/au" className="text-amber-700 hover:underline text-sm">
